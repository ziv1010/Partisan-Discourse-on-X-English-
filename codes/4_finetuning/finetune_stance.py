...
# # def main():
# #     ap = argparse.ArgumentParser(description="Few-shot stance classifier (entity/statement) — JSON output (batched + resume) with optional LoRA.")
...
# #     ap.add_argument("--trust_remote_code", action="store_true", help="Pass trust_remote_code=True to HF loaders (needed for some models, e.g., Mistral variants).")
...
# # Accepts older few-shot JSONs that may use positive/negative/neutral/for/against
# _LABEL_MAP_IN = {
#     "positive": "supports",
#     "negative": "denies",
#     "for": "supports",
...
# def main():
#     ap = argparse.ArgumentParser(description="Few-shot stance classifier (entity/statement) — JSON output (batched + resume) with optional LoRA and per-keyword shots.")
#     ap.add_argument("--input_csv", required=True, help="Input CSV with columns: tweet,...,keyword")
#     ap.add_argument("--model", required=True, help="Base model dir/checkpoint (decoder or encoder-decoder).")
...
#     mapped_col     = "fewshot_label_for_against"   # for | against | neutral | unrelated
#     reason_col     = "fewshot_reason"              # short explanation (from JSON)
#     shots_src_col  = "fewshot_shots_json"          # NEW: basename of the JSON used for this row
